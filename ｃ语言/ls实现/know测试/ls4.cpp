/*************************************************************************
	...
	> Author: ldc
	> Mail: litesla
	> Created Time: 2018年09月22日 星期六 14时17分22秒
 ************************************************************************/
...
void  do_ls(char *dirname, int op){
    DIR *dir_ptr;
...
void dostat(char *dirname,int i){
    ...
    if(stat(pwdname, &info) == -1){
        ...
        files[i].nlink = (int)info.st_nlink;
        files[i].size = (long)info.st_size;
        strcpy(files[i].ctim, 4 + ctime(&info.st_mtime));
        total += info.st_blocks/2;
        forlen.links = forlen.links < log10((int)info.st_nlink) ? log10((int)info.st_nlink) : forlen.links;
